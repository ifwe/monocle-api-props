...
function listPropsFromObject(resource, prefix) {
    ...
    for (var name in resource) {
        if (!Object.hasOwnProperty.call(resource, name)) {
            continue;
        }
        ...
        if (Array.isArray(resource[name])) {
            found = listPropsFromArray(resource[name], prefix + name + '@');
        } else if (typeof resource[name] === 'object' && null !== resource[name]) {
            found = listPropsFromObject(resource[name], prefix + name + '.');
        }
...
var objectHasChildProp = function(object, property) {
    return object && property && Object.hasOwnProperty.call(object, property) && typeof object[property] !== 'undefined';
};
...
MonocleProps.prototype.set = function(fullPath, value) {
    ...
    parents.forEach(function(item) {
        switch (last.type) {
            case 'object':
                if (Object.hasOwnProperty.call(item, last.property)) {
            ...
            case 'array':
                ...
                if (!last.property) {
                    ...
                    item.forEach(function(innerItem) {
                        if (Object.hasOwnProperty.call(innerItem, last.property)) {
...
MonocleProps.prototype.get = function(fullPath) {
    var paths = tokenize(fullPath);
    var all = getAll(this._resource, paths);
    debug('get', fullPath, all);
    return all;
};
...
function getAll(object, paths) {
    ...
    if (paths.length) {
        ...
        for (var i = 0, len = paths.length; i < len; i++) {
            ...
            switch (path.type) {
                case 'object':
                    debug('getting property from object', path.property, current);
                    if (!objectHasChildProp(current, path.property)) {
                        debug('did not find property in object', path.property, current);
                        return all;
                    }
                    current = current[path.property];
                    break;
                ...
                case 'array':
                    debug('plucking property from objects', path.property, current);
                    return getAllEach(current, paths.slice(i));
            }
        }
    ...
    debug('got all', object, paths, all);

    return all;
}

function getAllEach(array, paths) {
    debug('getAllEach', array, paths);
    var all = [];
    paths[0].type = 'object';

    for (var i = 0, len = array.length; i < len; i++) {
        all = all.concat(getAll(array[i], paths));
    }
